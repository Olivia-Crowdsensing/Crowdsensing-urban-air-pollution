...
def reduce_dimensions_train(df, dim, flag=True):
    ...
    # step0: Now, convert from the 500 -> original version 525
    f_train_set_s = set(range(0, len(training_set_s[0])))  # This is the full index set of training_set_s
    exc_training_set_s = f_train_set_s - know_mask  # These are index not in training, value of these should assign 999
    df_result = pd.DataFrame([know_mask, dim_reduced]).T  # This contains 500 known points
    df_exc = pd.DataFrame([exc_training_set_s, np.zeros((len(exc_training_set_s), dim)) + 999]).T

    # 2. concat the 1 & 2 together (df)
    df_trains = pd.concat([df_result, df_exc], axis=0, ignore_index=True)  # This is the 525.
    df_trains.sort_values(0, inplace=True)  # resort and form from 0 - 525
    df_trains.columns = ["index", "value"]

    # Now, to expand from 525 to 625
    # [know_set] step 1: from 525 -> 625
    ...
    df_0_exc = pd.DataFrame([exc_training_set, np.zeros((len(exc_training_set), dim)) + 999]).T
...
def visualization_fm_train(reduced_dim_m, dim, plot_dim_index):
    if dim == 3:
        v_list_0, v_list_1, v_list_2 = [], [], []
        for i in range(0, reduced_dim_m.shape[0]):
            v_list_0.append(reduced_dim_m["value"][i][0])
            v_list_1.append(reduced_dim_m["value"][i][1])
            v_list_2.append(reduced_dim_m["value"][i][2])
    elif dim == 2:
        v_list_0, v_list_1 = [], []
        for i in range(0, reduced_dim_m.shape[0]):
            v_list_0.append(reduced_dim_m["value"][i][0])
    ...
    if plot_dim_index == 0:
        v_list = v_list_0
    elif plot_dim_index == 1:
        v_list = v_list_1
    elif plot_dim_index == 2:
        v_list = v_list_2

    reduced_X_meshg = np.asarray(v_list).reshape(25, 25)
    reduced_X_meshg[reduced_X_meshg == 999] = np.nan  # replace 999 -> nan
    return reduced_X_meshg


if __name__ == '__main__':
    dataset = "synthetic"  # `pm25_BJ`; `pm25_China`
    # v_df = pd.read_excel("data/BJ/2018_Daily_AverageT2.xlsx", header=0)
    v_df = pd.read_excel("data/Synthetic/zeroAuto_zeroHete.xlsx", header=0)
    ...
    for epoch in range(370):
        ...
        for i in range(training_set.shape[0] // (t * batch_size)):  # <IGNNK  alg1. l2 (for sample = 1:S do)>
            # 实际上是对时间轴上的分割，因为training_set[0]是时间维度
            # print("subgraph #", i)
            ...
            inputs = np.array(feed_batch)  # observed value in this step
            ...
            X_h1, X_h2, X_est = GSTKriging_Model(Mf_inputs, A_q, A_h)  # Obtain the reconstruction (X_h1 not permute)
        ...
        # This is to save parameters and load
        # torch.save(GSTKriging_Model.state_dict(), "syn_model_parameter.pth")
        # torch.load("syn_model_parameter.highAuto.highHete.pth")

        # print("\n test graph")
        # MAE_t, RMSE_t, _, pred_pts, truth_pts = test_error(GSTKriging_Model, unknow_set,
...
# name = "zeroAuto.zeroHete"
# reduce_dimensions_train(outputs, dim=1, flag=False).to_excel("features/" + name + ".gT.xlsx")
# reduce_dimensions_train(X_est, dim=1, flag=False).to_excel("features/" + name + ".est.xlsx")
# reduce_dimensions_train(X_h1, dim=3).to_excel("features/" + name + ".hidden1.xlsx")
# reduce_dimensions_train(X_h2, dim=3).to_excel("features/" + name + ".hidden2.xlsx")